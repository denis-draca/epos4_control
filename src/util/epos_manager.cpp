...
EposManager::EposManager(hardware_interface::ActuatorStateInterface& asi,
    ...
    : asi_(&asi), avi_(&avi), api_(&api), jsi_(&jsi), jpi_(&jpi), jvi_(&jvi), jei_(&jei)
{
    BOOST_FOREACH(const std::string& motor_name, motor_names)
    {
        ...
        boost::shared_ptr<Epos> motor(new Epos(motor_name, nh, motor_config_nh, &epos_factory, *asi_, *avi_, *api_, *jsi_, *jpi_, *jvi_, *jei_));
