...
Epos::Epos(const std::string& name,
  ...
  : name_(name), config_nh_(config_nh), diagnostic_updater_(nh, config_nh), epos_factory_(epos_factory),
    has_init_(false),
    jposition_(0), jvelocity_(0), jeffort_(0),
    position_(0), velocity_(0), effort_(0), current_(0), statusword_(0),
    position_cmd_(0), velocity_cmd_(0), effort_cmd_(0), jposition_cmd_(0)
{
    valid_ = true;
    if(!config_nh_.getParam("joint_name", joint_name_))
    {
        ROS_ERROR("You must specify a joint name");
        valid_ = false;
    }
    ...
    hardware_interface::JointStateHandle jstate_handle(joint_name_, &jposition_, &jvelocity_, &jeffort_);
    jsi.registerHandle(jstate_handle);
    hardware_interface::JointHandle jposition_handle(jstate_handle, &jposition_cmd_);
    jpi.registerHandle(jposition_handle);
    hardware_interface::JointHandle jvelocity_handle(jstate_handle, &jvelocity_cmd_);
    jvi.registerHandle(jvelocity_handle);
    hardware_interface::JointHandle effort_handle(jstate_handle, &effort_cmd_);
...
bool Epos::init()
{
    ...
    ROS_WARN_STREAM("Initializing: 0x" << std::hex << serial_number_);
    unsigned int error_code;
    node_handle_ = epos_factory_->CreateNodeHandle("EPOS4", "MAXON SERIAL V2", "USB", serial_number_, &error_code);
    if(!node_handle_)
    {
...
void Epos::read()
{
    ...
    jposition_ = position_raw;
    jvelocity_ = velocity_raw;
    jeffort_ =  current_ * torque_constant_;
}
...
void Epos::write()
{
    ...
    {
        if(isnan(jposition_cmd_))
            return;

        ROS_INFO_STREAM("PROFILE_POSITION_MODE: pos=" << (int)jposition_cmd_ << " vel=" << (int)jvelocity_cmd_ << " eff=" << (int)effort_cmd_ << " node_id="<<node_handle_->node_id);

        VCS_MoveToPosition(node_handle_->device_handle->ptr, node_handle_->node_id, (int)jposition_cmd_, true, true, &error_code);
        //    ROS_INFO_STREAM("PROFILE_POSITION_MODE: error_code =" << error_code);
    }
}
